Feature: PBX Entity
  ...
  Scenario: Internal data is stored and hidden from queries
    ...
    And I write internal data for "test.asterisk.server" with payload '{"ariEndpoint":"http://172.27.255.250:8088"}'
    When I read internal data for "test.asterisk.server"
    Then the internal data matches '{"ariEndpoint":"http://172.27.255.250:8088"}'
